...
";
	...
	if ($backup_returnval == 0) {
		...
		my $encrypt_cmd = "nice openssl aes-128-cbc -e -salt -md md5 -k \"$CRYPTKEY\"";
...
sub is_compressed
{
    ...
    my $decrypt_cmd = "openssl aes-128-cbc -d -salt -md md5 -k \"$CRYPTKEY\"";
...
sub list_tarfile
{
	...
	#
	my $untar_cmd = "cat $tarfile";
	my $decrypt_cmd = "nice openssl aes-128-cbc -d -salt -md md5 -k \"$CRYPTKEY\"";
...
sub restore_files
{
	...
	if(grep(/^bbxd$/, @RESTORE)) {
		if($RTI == 0) {
			...
			system("cat $MOUNTPOINT/usr2.bak | openssl aes-128-cbc -d -salt -md md5 -k \"$CRYPTKEY\" | tar -C $ROOTDIR -xvf - usr2/bbx/bbxd 2>> $LOGFILE");
			$returnval += $?;
		}
	}
	...
	if(grep(/^bbxps$/, @RESTORE)) {
		if($RTI == 0) {
			...
			system("cat $MOUNTPOINT/usr2.bak | openssl aes-128-cbc -d -salt -md md5 -k \"$CRYPTKEY\" | tar -C $ROOTDIR -xvf - usr2/bbx/bbxps 2>> $LOGFILE");
			$returnval += $?;
		}
	}
...
sub validate_crypt_key
{
    ...
    my $decrypt_cmd = "openssl aes-128-cbc -d -salt -md md5 -k \"$cryptkey\" > /dev/null 2> /dev/null";
